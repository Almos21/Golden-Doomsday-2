...
namespace MoreMountains.CorgiEngine
{
    ...
    /// Animator parameters : Jetpacking (bool), JetpackTransformIn (trigger), JetpackTransformOut (trigger)
    /// </summary>
    ...
    public class CharacterJetpack : CharacterAbility
    {
        ...
        /// the duration of the transform-in animation (in seconds)
        [Tooltip("the duration of the transform-in animation (in seconds)")]
        public float TransformInDuration = 0.5f;
        /// the duration of the transform-out animation (in seconds)
        [Tooltip("the duration of the transform-out animation (in seconds)")]
        public float TransformOutDuration = 0.5f;
        /// whether to allow input during transformation
        [Tooltip("whether to allow input during transformation")]
        public bool AllowInputDuringTransformation = false;
        ...
        /// The sound to play during transformation in
        [Tooltip("The sound to play during transformation in")]
        public AudioClip TransformInSfx;
        /// The sound to play during transformation out
        [Tooltip("The sound to play during transformation out")]
        public AudioClip TransformOutSfx;
        ...
        protected bool _transformedIn = false;
        ...
        protected Coroutine _transformationCoroutine;

        // animation parameters
        protected const string _jetpackingAnimationParameterName = "Jetpacking";
        protected const string _transformInAnimationParameterName = "JetpackTransformIn";
        protected const string _transformOutAnimationParameterName = "JetpackTransformOut";
        protected int _jetpackingAnimationParameter;
        protected int _transformInAnimationParameter;
        protected int _transformOutAnimationParameter;
        ...
        {
            // Si estamos transformando y no permitimos input, retornamos
            if (IsTransforming && !AllowInputDuringTransformation)
            {
                return;
            }
        ...
        public virtual void JetpackStart()
        {
            ...
            // Si no estamos transformados, iniciamos la transformación
            if (!_transformedIn && !IsTransforming)
            {
                if (_transformationCoroutine != null)
                {
                    StopCoroutine(_transformationCoroutine);
                }
                _transformationCoroutine = StartCoroutine(TransformInCoroutine());
                return;
            }

            // Si ya estamos transformados, aplicamos la fuerza del jetpack
            if (_transformedIn)
            {
                ApplyJetpackForce();
            }
        ...
        /// <summary>
        /// Coroutine que maneja la animación de transformación de entrada
        /// </summary>
        protected virtual IEnumerator TransformInCoroutine()
        {
            IsTransforming = true;

            // Trigger de la animación de transformación
            if (_animator != null)
            {
                MMAnimatorExtensions.SetAnimatorTrigger(_animator, _transformInAnimationParameter, _character._animatorParameters, _character.PerformAnimatorSanityChecks);
            }

            // Reproducir sonido de transformación
            if (TransformInSfx != null)
            {
                MMSoundManagerSoundPlayEvent.Trigger(TransformInSfx, MMSoundManager.MMSoundManagerTracks.Sfx, this.transform.position);
            }

            // Esperar a que termine la animación
            yield return new WaitForSeconds(TransformInDuration);

            _transformedIn = true;
            IsTransforming = false;

            // Iniciar el jetpack automáticamente después de transformar
            ApplyJetpackForce();
        }

        /// <summary>
        /// Aplica la fuerza del jetpack
        /// </summary>
        protected virtual void ApplyJetpackForce()
        {
        ...
        public virtual void JetpackStop()
        {
            ...
            // Iniciar la transformación de salida
            if (_transformedIn && !IsTransforming)
            {
                if (_transformationCoroutine != null)
                {
                ...
                _transformationCoroutine = StartCoroutine(TransformOutCoroutine());
            }
            else
            {
        ...
        /// <summary>
        /// Coroutine que maneja la animación de transformación de salida
        /// </summary>
        protected virtual IEnumerator TransformOutCoroutine()
        {
            IsTransforming = true;

            // Trigger de la animación de transformación de salida
            if (_animator != null)
            {
                MMAnimatorExtensions.SetAnimatorTrigger(_animator, _transformOutAnimationParameter, _character._animatorParameters, _character.PerformAnimatorSanityChecks);
            }

            // Reproducir sonido de transformación de salida
            if (TransformOutSfx != null)
            {
                MMSoundManagerSoundPlayEvent.Trigger(TransformOutSfx, MMSoundManager.MMSoundManagerTracks.Sfx, this.transform.position);
            }

            // Esperar a que termine la animación
            yield return new WaitForSeconds(TransformOutDuration);

            _transformedIn = false;
            IsTransforming = false;
        ...
        {
            // Detener cualquier transformación en curso
            if (_transformationCoroutine != null)
            {
                StopCoroutine(_transformationCoroutine);
        ...
        protected override void InitializeAnimatorParameters()
        {
            ...
            RegisterAnimatorParameter(_transformInAnimationParameterName, AnimatorControllerParameterType.Trigger, out _transformInAnimationParameter);
            RegisterAnimatorParameter(_transformOutAnimationParameterName, AnimatorControllerParameterType.Trigger, out _transformOutAnimationParameter);
        }
